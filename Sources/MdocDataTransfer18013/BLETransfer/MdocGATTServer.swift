...
public class MdocGattServer: ObservableObject {
	...
	func handleStatusChange(_ newValue: TransferStatus) {
		...
		if newValue == .requestReceived {
			...
			let decodedRes = MdocHelpers.decodeRequestAndInformUser(deviceEngagement: deviceEngagement, docs: docs, iaca: iaca, requestData: readBuffer, devicePrivateKey: devicePrivateKey, readerKeyRawData: nil, handOver: BleTransferMode.QRHandover)
			switch decodedRes {
	...
	public func userSelected(_ b: Bool, _ items: RequestItems?) {
		...
		if let items {
			do {
				...
				guard let (drToSend, _, _) = try MdocHelpers.getDeviceResponseToSend(deviceRequest: deviceRequest!, deviceResponses: docs, selectedItems: items, sessionEncryption: sessionEncryption, eReaderKey: sessionEncryption!.sessionKeys.publicKey, devicePrivateKey: devicePrivateKey) else { errorToSend = MdocHelpers.getErrorNoDocuments(docTypeReq); return  }
