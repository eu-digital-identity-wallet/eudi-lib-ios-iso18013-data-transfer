...
extension MdocTransferManager {
	func getMdocResponseToSend(requestData: Data) -> Data? {
		guard let se = SessionEstablishment(data: [UInt8](requestData)) else  { logger.error("Eequest Data cannot be decoded to session establisment"); return nil }
		guard let deviceEngagement else { logger.error("Device Engagement not initialized"); return nil }
		sessionEncryption = SessionEncryption(se: se, de: deviceEngagement, handOver: .null)
