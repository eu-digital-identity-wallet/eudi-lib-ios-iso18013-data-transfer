...
extension MdocTransferManager {
	func getMdocResponseToSend(requestData: Data) -> Data? {
		do {
			...
			var docToSend = try getDeviceResponseToSend(deviceRequest, se: se) ?? DeviceResponse(status: 10)
	...
	func getDeviceResponseToSend(_ deviceRequest: DeviceRequest, se: SessionEstablishment) throws -> DeviceResponse? {
		...
		for docReq in deviceRequest.docRequests {
			...
			let authKeys = CoseKeyExchange(publicKey: se.eReaderKey!, privateKey: pk)
